...
These invariants are depicted in~\autoref{fig:invariantsfive}.
...
Now consider when all blocks have been placed. Then we have $i = \sum M_w -2$ and the gameboard will appear like in~\autoref{fig:afterplace}. The interval in invariant 4 will completely disappear and all terrain will be alternating apart from the interval mentioned in invariant 3.

Now only three possibilities exist. Let's consider how many cells can be cleared in each case.
...
\item $K < \sum M_w$ 
    ...
    \item Does not clear any white cells, since there aren't any left in column 3 to match those in column 4.
    \end{itemize}
