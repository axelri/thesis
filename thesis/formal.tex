...
\begin{quote}
\textbf{The (offline, no-rotation) k-cleared-cells problem}: Given a Lumines game $G$ and a goal $k \in \mathbb{N}$, does there exist a trajectory sequence $\phi$ such that when $\phi$ is applied to $G$, at least $k$ cells are cleared in total?
\end{quote}
